...
function createVHEdge(bool)
{
  ...
  return {
    ...
    contains: (z) => {
      ...
      isSelected = false//for highlighting
...
function createHVEdge(bool)
{
  ...
  return {
    ...
    contains: (z) => {
      p = center(end.getBounds())  // the values here are flipped from the VH edge for simplicities sake
      q = center(start.getBounds())
      isSelected = false//for highlighting
      //for when start node is to the left and below end node, vertical line
      if (p.x <= q.x && p.y >= q.y) {
        if (z.x > p.x - 10 && z.x < p.x + 10 && z.y < p.y - 10 && z.y > q.y + 10) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  graph.add(n4)
  graph.add(n5)
  const VHedge = createVHEdge(1)
  graph.connect(HVedge, { x: 160, y: 20 }, { x: 160, y: 110 })
  graph.connect(VHedge, { x: 75, y: 75}, {x: 400, y: 250 })
  graph.draw()
...
*/
/*
function createEdge(bool) {//controls flow and has property sheet options
  let start = undefined
  let end = undefined
  let p, q, HVedge, VHedge, solidEdge
  let isSelected = false
  let bend = 'Straight'
  const panel = document.getElementById('graphpanel')
  const line = document.createElementNS('http://www.w3.org/2000/svg', 'line')
  ...
  return {
    ...
    getBounds: () => {
      if(bend === 'VH')
      {
        VHEdge = createVHedge(bool, start, end, p, q, isSelected, bend, panel, line)
        return VHEdge.getBounds()
      ...
      else if(bend === 'HV')
      {
        ...
        solidEdge = createSolidEdge(bool, start, end, p, q, isSelected, bend, panel, line)
        return solidEdge.getBounds()
      }
    },
    contains: z => {
      if (bend === 'HV') {
        ...
        VHedge = createVHedge(bool, start, end, p, q, isSelected, bend, panel, line)
        return VHEdge.contains()
      } else {
        solidEdge = createSolidEdge(bool, start, end, p, q, isSelected, bend, panel, line)
        return solidEdge.contains(z)
    }
    },
    translate: () => { //does nothing
	  ...
	  if(bend === 'HV')
	  {
      let HVedge = createHVedge(bool, start, end, p, q, isSelected, bend, panel, line, vLine, hLine)
         HVedge.draw()
	  }
	  else if(bend === 'VH')
	  {
      let VHedge = createVHedge(bool, start, end, p, q, isSelected, bend, panel, line, vLine, hLine)
        VHedge.draw()
	  }
    else
    {
      let solidEdge = createSolidEdge(bool, start, end, p, q, isSelected, bend, panel, line)
        solidEdge.draw()
    }
    },
    clone: ()=>{
